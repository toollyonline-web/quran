...
const AyahItem: React.FC<AyahItemProps> = ({ verse, settings, onBookmark, isBookmarked, tafsir, tafsirName }) => {
  ...
  const getShareText = () => {
    ...
    if (settings.showEnglish && cleanEnglish) {
      shareText += `English: ${cleanEnglish}\n\n`;
    }
    
    if (settings.showUrdu && cleanUrdu) {
      shareText += `Urdu: ${cleanUrdu}\n\n`;
    }
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            <button 
              ...
              className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${showShareOptions ? 'text-emerald-600 bg-emerald-50' : 'text-slate-400 hover:bg-slate-100 hover:text-emerald-600'} dark:hover:bg-slate-800`}
            >
            ...
            {showShareOptions && (
              <div className="absolute right-0 top-10 z-10 w-48 rounded-2xl bg-white p-3 shadow-xl ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
                <ShareButtons 
                  variant="compact" 
